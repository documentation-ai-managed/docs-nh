...
config:
  theme: neutral
  layout: elk
title: Sales Pipeline & Automation Integration
---
flowchart TD
    ...
    %% Dùng shape [[ ]] để biểu thị Subroutine/Automation
    Auto1[["⚡ Webform Catch"]]
    Auto2[["⚡ Auto-Reclaim 2h"]]
    Auto3[["⚡ Auto-Create Deal"]]
    Auto4[["⚡ Sync Contract"]]
    ...
    %% 1. Định nghĩa Class
    %% Status Colors (Standard Luklak Palette)
    ...
    %% VIP Class (Gold/Yellow)
    classDef vip fill:#FFF9C4,stroke:#FBC02D,stroke-width:2px,color:#213459
    
    %% AUTOMATION CLASS (Distinct Purple Style)
    %% Màu tím nhạt nền, viền tím đậm, chữ tím đậm để nổi bật đây là SYSTEM làm
    classDef automation fill:#F3E5F5,stroke:#9C27B0,stroke-width:2px,stroke-dasharray: 5 5,color:#4A148C
    ...
    %% Apply Automation Class
    class Auto1,Auto2,Auto3,Auto4 automation

    %% Animation cho các đường dẫn tự động (Optional)
    linkStyle 1,3,4,8,9,13,14 stroke:#9C27B0,stroke-width:2px,stroke-dasharray: 3 3
```
